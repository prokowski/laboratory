...
class LaboratoryApplicationSpec extends Specification {
    ...
    @Autowired
    RackQueryRepository rackQueryRepository
    ...
    def "happy path for assignment samples to racks"() {
        ...
        mvc.perform(put("/api/racks/assignSample/{id}", "SMP1")).andExpect(status().isOk())

        mvc.perform(put("/api/racks/assignSample/{id}", "SMP2")).andExpect(status().isOk())

        mvc.perform(put("/api/racks/assignSample/{id}", "SMP3")).andExpect(status().isOk())

        mvc.perform(put("/api/racks/assignSample/{id}", "SMP4")).andExpect(status().isOk())
